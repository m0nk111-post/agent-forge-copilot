...
class InlineChatProvider {
    ...
    // Helper methods
    buildPrompt(instruction, selectedText, editor) {
        let prompt = '';
    ...
    async applyCode(editor, selection, code, originalText) {
        if (originalText) {
    ...
    async showDiff(uri, original, modified) {
        ...
        const choice = await vscode.window.showInformationMessage('Accept optimized code?', 'Accept', 'Reject');
        return choice === 'Accept';
    }
    getDocStyle(languageId) {
        const docStyles = {
            'typescript': 'TSDoc',
            'javascript': 'JSDoc',
            'python': 'docstring',
