...
class OllamaService {
    ...
    loadConfig() {
        ...
        const settingsUrl = config.get('ollamaUrl');
    ...
    async chatWithTools(userMessage, tools, workspacePath, referencedFiles) {
        // Wait for config to be ready
        if (this.configLoadPromise) {
            await this.configLoadPromise;
        }
        ...
        const toolDescriptions = tools.map(t => {
            ...
            const paramList = Object.entries(params)
                .map(([name, info]) => `${name}: ${info.description || 'string'}`)
                .join(', ');
...
User request: ${userMessage}`;
    ...
    async analyzeImage(imageDataUrl, prompt = "Describe this image in detail.") {
        try {
            // Wait for config to be ready
            if (this.configLoadPromise) {
                await this.configLoadPromise;
