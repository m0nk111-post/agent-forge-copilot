...
import { CompletionProvider, CodeActionProvider, HoverProvider, SignatureHelpProvider } from './services/completionProvider';
import { CommitMessageProvider, PRDescriptionProvider, CodeReviewProvider, FormatterProvider } from './services/advancedProviders';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    try {
        ...
        const configManager = new ConfigurationManager(context.extensionPath);
        ...
        const ollamaService = new OllamaService(context.extensionPath);
        const fileService = new FileService();
        const terminalService = new TerminalService();
        const gitService = new GitService();
        const mcpService = new MCPService();
        const agentService = new AgentService(ollamaService, fileService, terminalService, configManager);
        const inlineChatProvider = new InlineChatProvider(ollamaService, configManager);
        ...
        const completionProvider = new CompletionProvider(ollamaService, configManager);
        const inlineCompletionProvider = new InlineCompletionProvider(ollamaService, fileRelationshipTracker); // NEW: Ghost text completions with multi-file context
        const codeActionProvider = new CodeActionProvider(ollamaService, configManager);
        const hoverProvider = new HoverProvider(ollamaService, configManager);
        const signatureHelpProvider = new SignatureHelpProvider(ollamaService, configManager);
        const commitMessageProvider = new CommitMessageProvider(ollamaService, gitService, configManager);
        const prDescriptionProvider = new PRDescriptionProvider(ollamaService, gitService, configManager);
        const codeReviewProvider = new CodeReviewProvider(ollamaService, configManager);
        const formatterProvider = new FormatterProvider(ollamaService, configManager);
            ...
            ollamaService.checkConnection().then(connected => {
                if (connected) {
                    ...
                    vscode.window.showWarningMessage(
                        '⚠️ Agent Forge loaded, but cannot connect to Ollama. Please check settings.',
                        'Open Settings'
                    ).then(choice => {
    ...
    // Register commands
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.openChat', () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.inlineCompletionAccepted', () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.explainCode', async () => {
            ...
            const explanation = await ollamaService.chat(
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.refactorCode', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return;
            ...
            await vscode.window.withProgress({
                ...
                const refactored = await ollamaService.chat(
                    `Refactor this code for better readability, performance, and maintainability. Return ONLY the refactored code without explanations:\n\n\`\`\`${editor.document.languageId}\n${selection}\n\`\`\``
                );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.generateTests', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return;
            ...
            await vscode.window.withProgress({
                ...
                const tests = await ollamaService.chat(
                    `Generate comprehensive unit tests for this code. Use the appropriate testing framework for ${editor.document.languageId}. Include edge cases:\n\n\`\`\`${editor.document.languageId}\n${selection}\n\`\`\``
                );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.fixBug', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) return;
            ...
            await vscode.window.withProgress({
                ...
                const fixed = await ollamaService.chat(
                    `Analyze this code for bugs and fix them. Explain what was wrong and how you fixed it. Then provide the fixed code:\n\n\`\`\`${editor.document.languageId}\n${selection}\n\`\`\``
                );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.showConfig', async () => {
            await configManager.showConfigurationUI();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.reloadConfig', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.showModelInfo', () => {
            const config = vscode.workspace.getConfiguration('agent-forge');
            const modelName = config.get<string>('model', 'No model selected');
            const ollamaUrl = config.get<string>('ollamaUrl', 'http://localhost:11434');
            
            vscode.window.showInformationMessage(
                `🤖 Agent Forge\n\nActive Model: ${modelName}\nOllama Server: ${ollamaUrl}`,
                'Change Model',
                'Open Settings'
            ).then(choice => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.runTerminalCommand', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.agentMode', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.configureAgent', async () => {
            ...
            if (choice.startsWith('Auto-approve')) {
                ...
                vscode.window.showInformationMessage(
                    `Agent auto-approve ${newValue ? 'ENABLED' : 'DISABLED'}`
                );
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.inlineChat', async () => {
            await inlineChatProvider.showInlineChat();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.quickFix', async () => {
            await inlineChatProvider.quickFix();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.quickExplain', async () => {
            await inlineChatProvider.quickExplain();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.quickOptimize', async () => {
            await inlineChatProvider.quickOptimize();
        })
    );

    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.quickDocument', async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.generateCommit', async () => {
            await vscode.window.withProgress({
                location: vscode.ProgressLocation.Notification,
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.generatePR', async () => {
    ...
    // Review code
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.reviewCode', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
            ...
            await vscode.window.withProgress({
                ...
                try {
                    const review = await codeReviewProvider.reviewCode(
                        editor.document,
                        !selection.isEmpty ? selection : undefined
                    );
    ...
    // Review workspace
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.reviewWorkspace', async () => {
            const choice = await vscode.window.showWarningMessage(
                'This will review multiple files in your workspace. Continue?',
                'Yes', 'No'
            );
            ...
            await vscode.window.withProgress({
                ...
                try {
                    ...
                    channel.show();
                    vscode.window.showInformationMessage(
                        `Workspace review complete: ${results.totalFiles} files, ${results.totalIssues} issues`
                    );
                } catch (error: any) {
    ...
    // Format with AI
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.formatDocument', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
                vscode.window.showErrorMessage('No active editor');
                return;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.fixDiagnostic', 
...
Provide ONLY the fixed code for lines ${range.start.line + 1}-${range.end.line + 1}. No explanations, no markdown, just the corrected code.`;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.explainDiagnostic',
            async (document: vscode.TextDocument, diagnostic: vscode.Diagnostic) => {
                await vscode.window.withProgress({
                    location: vscode.ProgressLocation.Notification,
...
<body>
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.extractFunction',
...
Return ONLY the function definition and the call, separated by a blank line. No explanations.`;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.addDocumentation',
            async (document: vscode.TextDocument, range: vscode.Range) => {
                const editor = await vscode.window.showTextDocument(document);
...
Return the code WITH documentation comments added. Keep the code exactly the same, only add comments.`;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.optimizeCode',
            async (document: vscode.TextDocument, range: vscode.Range) => {
                const editor = await vscode.window.showTextDocument(document);
                const code = document.getText(range);
...
Return ONLY the optimized code. No explanations.`;
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.toggleCompletions', async () => {
            const config = vscode.workspace.getConfiguration('agent-forge');
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('agent-forge.clearCache', () => {
