...
export class ConfigurationManager implements vscode.Disposable {
    ...
    private guidelinesContent: string = '';
    ...
    private loadDefaultConfig(): AgentConfiguration {
    ...
    private async loadGuidelinesFile() {
    ...
    async setGuidelinesFile(filePath: string) {
        const vsconfig = vscode.workspace.getConfiguration('agent-forge');
    ...
    private async selectGuidelinesFile() {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
    ...
    private async setCustomInstructions() {
    ...
    private async toggleAutoApprove() {
    ...
    private async setMaxIterations() {
        const input = await vscode.window.showInputBox({
